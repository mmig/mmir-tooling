 ...
 * 
 * @param {string} path
 * 					the path to the file, for which to create directories (including the file itself)
 * @param {Function} [callback]
 * 					a callback function: if provided, the directory-creation will be done asynchronously
 * @param {boolean} [isDebugOutput]
 * 					if true, debug information will be printed to console
 * @param {string} [pathSep]
 * 					If 'default', the platform's default path separator will be used.
 * 					If omitted (or FALSY), '/' will be used.
 * 					Otherwise the provided string will be used.
 */
module.exports = function(path, callback, isDebugOutput, pathSep){
	...
	var mkDirList = [];
	var lastIndex = path.lastIndexOf(pathSep);
	if(lastIndex !== -1){
		...
		if( ! isDirExisting){
			...
			lastIndex = dirPath.lastIndexOf(pathSep);
			while(lastIndex !== -1){
